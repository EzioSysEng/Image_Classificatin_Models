...
def train(epoch):
    ...
    net.train()
    for batch_index, (img, label) in enumerate(cifar100_training_loader):
        if epoch <= args.warm:
        ...
        print('Training Epoch: {epoch} [{trained_samples}/{total_samples}]\tLoss: {:0.4f}\tLR: {:0.6f}'.format(
            ...
            epoch=epoch,
            trained_samples=batch_index * args.b + len(img),
...
def eval_training(epoch):
    ...
    for (img, label) in cifar100_test_loader:
        img = Variable(img)
        label = Variable(label)
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(log_dir=os.path.join(settings.LOG_DIR))
